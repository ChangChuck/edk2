/** @file
  Port of Ps2KeyboardDxe from IntelFrameworkModulePkg to use ARM PL050 instead
  of Intel/IBM 8042
...
/**
  Timer event handler: read a series of scancodes
...

{
  ...
  while (KB_READ_REG8 (KMI_STAT) & KMI_STAT_RX_FULL) {
    //
    // Read one byte of the scan code and store it into the memory buffer
    //
    Data = KB_READ_REG8 (KMI_DATA);
...

{
  ...
  for (TimeOut = 0; TimeOut < KEYBOARD_TIMEOUT; TimeOut += 30) {
    if (KB_READ_REG8 (KMI_STAT) & KMI_STAT_RX_FULL) {
      RegFilled = 1;
      *Data     = KB_READ_REG8 (KMI_DATA);
      break;
    }
  ...
  )
{
  ...
  //
  for (TimeOut = 0; TimeOut < KEYBOARD_TIMEOUT; TimeOut += 30) {
    if (KB_READ_REG8 (KMI_STAT) & KMI_STAT_TX_EMPTY) {
      RegEmptied = 1;
      break;
    }
...
/**
  ...
  Data register by keyboard and then read it,
  ...
  )
{
  ...
  //
  // Read from data register (multiple times if needed)
  ...
  while (1) {
    ...
    DEBUG ((EFI_D_ERROR, "Wait for %08x got %08x\n", Value, Data));
...
/**
  Perform controller and keyboard Initialization.
  ...
  )
{
  ...
  // Enable PL050
  //
  KB_WRITE_REG8 (KMI_CR, KMI_CR_EN);

  //
  // Keyboard reset with a BAT(Basic Assurance Test)
  //
  Status = KeyboardWrite (ConsoleIn, KEYBOARD_COMMAND_RESET);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "PS2 keyboard controller data write error!\n\r"));
    goto Done;
  }

  Status = KeyboardWaitForValue (ConsoleIn, KEYBOARD_RETURN_ACK);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "Some specific value not aquired from PS2 keyboard controller!\n\r"));
    goto Done;
  }
  //
  ...
  Status                = KeyboardWaitForValue (ConsoleIn, KEYBOARD_RETURN_BAT_SUCCESS);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "Keyboard self test failed!\n\r"));
    goto Done;
  }
  ...
  //
  Status = KeyboardWrite (ConsoleIn, KEYBOARD_COMMAND_SELECT_SCAN_CODE_SET);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "PS2 keyboard controller data write error!\n\r"));
    goto Done;
  }
  ...
  Status = KeyboardWrite (ConsoleIn, 0x01);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "PS2 keyboard controller data write error!!\n\r"));
    goto Done;
  }
  ...
  //
  Status = KeyboardWrite (ConsoleIn, KEYBOARD_COMMAND_CLEAR_OUTPUT_DATA);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "PS2 keyboard controller data write error!\n\r"));
    goto Done;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "Update keyboard status lights error!\n\r"));
  }
